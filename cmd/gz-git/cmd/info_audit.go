...
func buildAudit(
	...
	for i := range result.Repositories {
		...
		repos = append(repos, repository.EvaluateRepo(repository.AuditInput{
			...
			Status:                     status,
			Base:                       enr.Base,
			IntegrationName:            enr.Integration.Name,
			IntegrationSource:          enr.Integration.Source,
			UpstreamTargetsIntegration: enr.UpstreamTargetsIntegration,
			UpstreamRemote:             enr.UpstreamRemote,
			TaskRemoteExists:           enr.TaskRemoteExists,
