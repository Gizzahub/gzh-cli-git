...
Prepared release notes for 0.8.0 (2026-08-25). Publication remains pending until the
Homebrew tap is initialized, its scoped token is configured, and the stable release
workflow and macOS Cask installation pass. New unreleased changes live in
[CHANGELOG.md](../../CHANGELOG.md).

______________________________________________________________________

## 0.8.0 candidate — publication pending

<!-- 2026-08-07 .. 2026-08-25. Reconstructed from commit history after this file went
     unmaintained for two weeks; grouped by theme rather than by commit. -->

### Added

- `workspace sync` now reconciles an explicit repo-root `integrationBranch` into
  repository-local hook participation after successful clone/update actions. A managed
  marker makes interrupted writes recoverable and prevents manual settings from being
  adopted, overwritten, or removed; plain `git clone` remains an explicit manual setup.
- Workspace에 `access: read-only`를 선언하면 다른 소유자의 저장소를 계속
  clone/pull하면서도 모든 bulk push, sync 후 push 및 handoff checkpoint에서
  제외할 수 있습니다. forge/config workspace의 하위 저장소와 심볼릭 링크 대상도
  같은 계약을 상속하며, 외부 절대경로 저장소에는 동기화 후에도 유지되는 로컬 Git
  access marker를 기록합니다.
  읽기 전용 workspace의 sync 전략은 명령행 override보다 우선하여 항상 `pull`로
  고정됩니다.
...
- A bare `integrate check` or `integrate run` from the resolved target checkout now exits
  1 with a task-worktree diagnosis. Explicit source arguments and same-SHA task branches
  retain their existing behavior, including reclaim recovery after an earlier landing.
- Integration still succeeds for `hotfix/*` and `release/*`, but automatic reclaim now
  preserves their worktree, local branch, and remote branch even if `taskPattern` matches.
  These protected namespaces require explicit follow-up cleanup after integration.
...
> The review follow-ups named in the reconstructed notes were resolved before the
> v0.8.0 candidate was prepared. Their original findings and verification evidence
> remain in `tasks/HISTORY.md` and the corresponding archived issue files.

### Release engineering

- Updated the development, workspace, and CI toolchain pins to Go 1.26.7.
- Pinned GoReleaser to v2.10.2 and migrated Homebrew publishing from the deprecated
  Formula configuration to a macOS Cask, with snapshot generation and Ruby syntax
  validation covering the generated Cask before publication.
- Kept the moving snapshot tag out of stable-version discovery. The first stable tag
  remains gated on bootstrapping gizzahub/homebrew-tap, configuring the least-privilege
  HOMEBREW_TAP_TOKEN, and completing an actual Cask install.
