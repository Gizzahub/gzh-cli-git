...
out=$(PATH="$good:$BASE_PATH" "$audit" gz-git "$good/gz-git" 2>&1)
...
out=$(PATH="$same_ver_shadow:$good:$BASE_PATH" "$audit" gz-git "$good/gz-git" 2>&1)
...
out=$(PATH="$good:$shadow:$BASE_PATH" "$audit" gz-git "$good/gz-git" 2>&1)
check duplicate 1 "다른 버전의 'gz-git' 가 PATH 에 있다" "$?" "$out"

# 4) WARN_ONLY — 같은 결함을 보고하되 실패시키지 않는다.
out=$(INSTALL_AUDIT_WARN_ONLY=1 PATH="$good:$shadow:$BASE_PATH" "$audit" gz-git "$good/gz-git" 2>&1)
...
out=$(RECLAIM=1 PATH="$good:$impostor:$BASE_PATH" "$audit" gz-git "$good/gz-git" 2>&1)
rc=$?
...
out=$(RECLAIM=1 PATH="$good:$reclaim:$BASE_PATH" "$audit" gz-git "$good/gz-git" 2>&1)
rc=$?
...
make_fake "$off" 0.7.0
out=$(PATH="$BASE_PATH" "$audit" gz-git "$off/gz-git" 2>&1)
