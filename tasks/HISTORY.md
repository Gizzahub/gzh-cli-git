...
| #   | 태스크                                                                                                       | 우선순위 | 요지                                                                                                                                                          |
| --- | ------------------------------------------------------------------------------------------------------------ | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 28  | [goreleaser-brews-deprecated-and-ci-pins-latest](issue/28-goreleaser-brews-deprecated-and-ci-pins-latest.md) | P1       | GoReleaser `v2.10.2` 고정, Cask 이전, snapshot tag 제외와 생성 Cask Ruby 구문 게이트를 구현해 설정 폐기·재현성 위험을 해소 (`24944dc`·`5be5005`·`5cbd428`) ✅ |

검증: `goreleaser check`, 실제 snapshot release, 생성 Cask의 `ruby -c`, 깨끗한 clone·모듈
캐시의 snapshot artifact 5개, `GOWORK=off make quality-check`, `make check`·`make lint`가
통과했다. 외부 `gizzahub/homebrew-tap`에서 기존 Formula→Cask 업그레이드를 검증하고 기존
Formula를 정리하는 일은 릴리스 운영 선행조건으로 분리했다.
