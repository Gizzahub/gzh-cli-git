...
> 아래 내용은 수정 전 결함의 관측·재현·설계 판단을 보존한 역사 기록이다. 현재 동작은
> 위 [해결](#%ED%95%B4%EA%B2%B0) 절을 따른다. 이 절의 현재형 표현은 발견 당시 상태를 가리킨다.
...
파일링 당시 이 이슈는 결함 보고였고 구현 결정은 포함하지 않았다. 다만 초판이 "`info`에 붙일지
...
- upstream 관계를 분류하는 자리도 이미 있다 — `evaluateUpstream`
  ...
  자연스럽게 붙는 다섯 번째 상태다.
- 착수 전에 확인했던 것: `info`/`info_audit`은 `effective.Branch.DefaultBranch`를
